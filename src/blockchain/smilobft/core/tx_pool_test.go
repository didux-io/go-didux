...
var minBalanceForSmiloPay = big.NewInt(100000000)
...
func setupSmiloTxPool() (*TxPool, *ecdsa.PrivateKey) {
	db := rawdb.NewMemoryDatabase()
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(db))
	blockchain := &testBlockChain{statedb, statedb, 1000000, new(event.Feed)}

	key, _ := crypto.GenerateKey()
	pool := NewTxPool(testTxPoolConfig, params.SmiloTestChainConfig, blockchain)
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	// setup testChain
	statedb.SetBalance(address, minBalanceForSmiloPay, big.NewInt(0))
	statedb.SetSmiloPay(address, minBalanceForSmiloPay)
...
func TestInvalidTransactions(t *testing.T) {
	...
	pool.currentState.AddBalance(from, big.NewInt(1), big.NewInt(1))
	...
	pool.currentState.AddBalance(from, big.NewInt(0xffffffffffffff), big.NewInt(1))
...
//Test for transactions that are invalid on Smilo XSM, considering ErrOversizedData errs
func TestInvalidTransactionsCustomTransactionSizeLimit(t *testing.T) {
	pool, key := setupTxPool()
	defer pool.Stop()
	...
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(rawdb.NewMemoryDatabase()))
	blockchain := &testBlockChain{statedb, statedb, 1000000, new(event.Feed)}
	params.TestChainConfig.CustomTransactionSizeLimit = 128
	...
	tx5, _ := types.SignTx(types.NewTransaction(2, common.Address{}, big.NewInt(100), 100000, big.NewInt(1), data3), types.HomesteadSigner{}, key)
	if err := pool2.AddRemote(tx5); err != ErrOversizedData {
		t.Error("expected", ErrOversizedData, "; got", err)
	}

	tx6, _ := types.SignTx(types.NewTransaction(1, common.Address{}, big.NewInt(100), 0, big.NewInt(0), nil), types.HomesteadSigner{}, key)
	balance := new(big.Int).Add(tx6.Value(), new(big.Int).Mul(new(big.Int).SetUint64(tx5.Gas()), tx5.GasPrice()))
...
//Test for transactions that are only invalid on Smilo
func TestSmiloInvalidTransactions(t *testing.T) {
...
func TestTransactionQueue(t *testing.T) {
	...
	tx := transaction(0, 100, key)
	from, _ := deriveSender(tx)
	pool.currentState.AddBalance(from, big.NewInt(1000), big.NewInt(1))
	<-pool.requestReset(nil, nil)
...
func TestTransactionMissingNonce(t *testing.T) {
	...
	addr := crypto.PubkeyToAddress(key.PublicKey)
	pool.currentState.AddBalance(addr, big.NewInt(100000000000000), big.NewInt(1))
...
func TestTransactionDropping(t *testing.T) {
	...
	pool.currentState.AddBalance(account, big.NewInt(1000), big.NewInt(1))
	...
	pool.currentState.AddBalance(account, big.NewInt(-650), big.NewInt(1))
	<-pool.requestReset(nil, nil)
...
func TestTransactionPostponing(t *testing.T) {
	...
	for i := 0; i < len(keys); i++ {
		...
		pool.currentState.AddBalance(crypto.PubkeyToAddress(keys[i].PublicKey), big.NewInt(50100), big.NewInt(1))
	}
	...
	for _, addr := range accs {
		pool.currentState.AddBalance(addr, big.NewInt(-1), big.NewInt(1))
	}
	<-pool.requestReset(nil, nil)
...
func TestTransactionGapFilling(t *testing.T) {
	...
	pool.currentState.SetSmiloPay(account, big.NewInt(1000000000))
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	for i := 0; i < len(keys); i++ {
		keys[i], _ = crypto.GenerateKey()
		pool.currentState.AddBalance(crypto.PubkeyToAddress(keys[i].PublicKey), big.NewInt(1000000), big.NewInt(1))
	}
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	localAddress := crypto.PubkeyToAddress(local.PublicKey)
	remoteAddress := crypto.PubkeyToAddress(remote.PublicKey)

	pool.currentState.AddBalance(localAddress, big.NewInt(10000000000000000), big.NewInt(1))
	pool.currentState.SetSmiloPay(localAddress, big.NewInt(2000000000))

	pool.currentState.AddBalance(remoteAddress, big.NewInt(10000000000000000), big.NewInt(1))
	pool.currentState.SetSmiloPay(remoteAddress, big.NewInt(2000000000))

	// Add the two transactions and ensure they both are queued up
	if err := pool.AddLocal(pricedTransaction(1, 100000, big.NewInt(1), local)); err != nil {
		t.Fatalf("failed to add local transaction: %v, address: %v", err, localAddress.String())
	}
	if err := pool.AddRemote(pricedTransaction(1, 100000, big.NewInt(1), remote)); err != nil {
		t.Fatalf("failed to add remote transaction: %v, address: %v", err, remoteAddress.String())
	}
	pending, queued := pool.Stats()
	if pending != 0 {
...
func testTransactionLimitingEquivalency(t *testing.T, origin uint64) {
	...
	pool1.currentState.AddBalance(account1, big.NewInt(10000000000000000), big.NewInt(1))

	pool1.currentState.SetSmiloPay(account1, big.NewInt(1000000000))
...
func TestTransactionPoolRepricingKeepsLocals(t *testing.T) {
	...
	for i := 0; i < len(keys); i++ {
		keys[i], _ = crypto.GenerateKey()
		pool.currentState.AddBalance(crypto.PubkeyToAddress(keys[i].PublicKey), big.NewInt(1000*1000000), big.NewInt(1))
	}
...
func TestTransactionReplacement(t *testing.T) {
	...
	key, _ := crypto.GenerateKey()
	pool.currentState.AddBalance(crypto.PubkeyToAddress(key.PublicKey), big.NewInt(1000000000), big.NewInt(1))
...
func testTransactionJournaling(t *testing.T, nolocals bool) {
	...
	pool.currentState.AddBalance(crypto.PubkeyToAddress(local.PublicKey), big.NewInt(1000000000), big.NewInt(1))
	pool.currentState.AddBalance(crypto.PubkeyToAddress(remote.PublicKey), big.NewInt(1000000000), big.NewInt(1))
