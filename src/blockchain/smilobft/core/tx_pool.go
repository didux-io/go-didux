...
var (
	...
	ErrInsufficientMinFunds = errors.New("insufficient funds, minimum required is 0.01")
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if pool.currentMaxGas < tx.Gas() {
		log.Debug("############### validateTx, exceeds block gas limit ", "pool.currentMaxGas", pool.currentMaxGas, "tx.Gas", tx.Gas(), "isGas", isGas, "TX-Hash", tx.Hash().Hex(), "GasPrice", tx.GasPrice())
		return ErrGasLimit
	}
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (replaced bool, err error) {
	isSmilo := pool.chainconfig.IsSmilo
	isGas := pool.chainconfig.IsGas
	...
	if uint64(pool.all.Count()) >= pool.config.GlobalSlots+pool.config.GlobalQueue {
		// If smilo node and isGas true
		// And If the new transaction is Under priced, don't accept it
		if !local && (!isSmilo || (isSmilo && isGas)) && pool.priced.Underpriced(tx, pool.locals) {
			log.Trace("Discarding underpriced transaction", "hash", hash, "price", tx.GasPrice(), "IsSmilo", pool.chainconfig.IsSmilo, "IsGas", pool.chainconfig.IsGas)
...
func (pool *TxPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		for _, tx := range olds {
			hash := tx.Hash()
			log.Trace("$$$$$$$$$$$$$$$$$ demoteUnexecutables, Removed old pending transaction", "hash", hash)
			pool.all.Remove(hash)
		}
		// Drop all transactions that are too costly (low balance or out of gas), and queue any invalids back for later

		costLimit := pool.currentState.GetBalance(addr)
		gasLimit := pool.currentMaxGas
		...
		for _, tx := range drops {
			hash := tx.Hash()
			log.Debug("$$$$$$$$$$$$$$$$$ demoteUnexecutables, Removed unpayable pending transaction", "hash", hash, "costLimit", costLimit, "gasLimit", gasLimit, "blockNum", blockNum, "smiloPayLimit", smiloPayLimit)
			pool.all.Remove(hash)
		}
		...
		for _, tx := range invalids {
			hash := tx.Hash()
			log.Warn("$$$$$$$$$$$$$$$$$ demoteUnexecutables, Demoting pending transaction", "hash", hash, "costLimit", costLimit, "gasLimit", gasLimit, "blockNum", blockNum, "smiloPayLimit", smiloPayLimit)
