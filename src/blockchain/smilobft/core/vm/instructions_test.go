...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase, opFn executionFunc, name string) {
	...
	for i, test := range tests {
		...
		stack.push(x)
		stack.push(y)
		opFn(&pc, evmInterpreter, nil, nil, stack, false)
		actual := stack.pop()
...
func getResult(args []*twoOperandParams, opFn executionFunc) []TwoOperandTestcase {
	...
	for i, param := range args {
		...
		stack.push(x)
		stack.push(y)
		opFn(&pc, interpreter, nil, nil, stack, false)
		actual := stack.pop()
...
func opBenchmark(bench *testing.B, op func(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack, isVault bool) ([]byte, error), args ...string) {
	var (
		env            = NewEVM(Context{}, nil, nil, params.TestChainConfig, Config{})
		stack          = newstack()
	...
	for i := 0; i < bench.N; i++ {
		...
		op(&pc, evmInterpreter, nil, nil, stack, false)
		stack.pop()
	}
	poolOfIntPools.put(evmInterpreter.intPool)
}
...
func TestOpMstore(t *testing.T) {
	...
	opMstore(&pc, evmInterpreter, nil, mem, stack, false)
...
func BenchmarkOpSHA3(bench *testing.B) {
	...
	for i := 0; i < bench.N; i++ {
		...
		opSha3(&pc, evmInterpreter, nil, mem, stack, false)
	}
	poolOfIntPools.put(evmInterpreter.intPool)
}
