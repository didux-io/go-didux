...
func TestCall(t *testing.T) {
	...
	ret, _, err := Call(address, nil, &Config{State: state}, false)
	if err != nil {
		t.Fatal("didn't expect error", err)
	}
...
func BenchmarkCall(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for j := 0; j < 400; j++ {
			Execute(code, cpurchase, nil, false)
			Execute(code, creceived, nil, false)
			Execute(code, refund, nil, false)
		}
	}
}
func benchmarkEVM_Create(bench *testing.B, code string) {
	...
	bench.ResetTimer()
	for i := 0; i < bench.N; i++ {
		Call(receiver, []byte{}, &runtimeConfig, false)
	}
	bench.StopTimer()
}
