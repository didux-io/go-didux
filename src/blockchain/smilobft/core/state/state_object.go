...
// Account is the Didux consensus representation of accounts.
...
func (s *stateObject) SetSmiloPay(amount *big.Int) {
	s.db.journal.append(smiloPayChange{
		account: &s.address,
		prev:    new(big.Int).Set(s.data.SmiloPay),
	})
	s.setSmiloPay(amount)
}

func (s *stateObject) UpdateSmiloPay(blockNumber *big.Int) {
	prevsmiloPay := s.data.SmiloPay
	prevblock := s.data.BlockNumber
	smiloPay := CalculateSmiloPay(prevblock, blockNumber, prevsmiloPay, s.data.Balance)
	s.db.journal.append(blockChange{
		account:      &s.address,
		prevSmiloPay: prevsmiloPay,
		prevblock:    prevblock,
	})
	s.setSmiloPayAndBlock(smiloPay, blockNumber)
}
...
func (s *stateObject) setSmiloPay(amount *big.Int) {
	s.data.SmiloPay = amount
}

func (s *stateObject) setSmiloPayAndBlock(amount *big.Int, blockNumber *big.Int) {
	s.data.SmiloPay = amount
...
func (s *stateObject) SmiloPay() *big.Int {
	return s.data.SmiloPay
}
...
func (s *stateObject) BlockNumber() *big.Int {
