...
func (self *StateDB) GetSmiloPay(addr common.Address, blockNumber *big.Int) *big.Int {
	stateObject := self.getStateObject(addr)
	ret := common.Big0
	if stateObject != nil {
		ret = CalculateSmiloPay(stateObject.BlockNumber(), blockNumber, stateObject.SmiloPay(), stateObject.Balance())
	}
	//fmt.Println("Available SmiloPay: ", ret.Int64())
	return ret
}
...
func (self *StateDB) AddSmiloPay(addr common.Address, amount *big.Int) {
	stateObject := self.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.AddSmiloPay(amount)
	}
}
...
func (self *StateDB) SubSmiloPay(addr common.Address, amount, blockNumber *big.Int) {
	stateObject := self.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SubSmiloPay(amount, blockNumber)
	}
}
...
func (self *StateDB) SetSmiloPay(addr common.Address, amount *big.Int) {
	stateObject := self.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SetSmiloPay(amount)
	} else {
		fmt.Println("****** KARAIDIASA ******** ")
	}

}

func (self *StateDB) UpdateSmiloPay(addr common.Address, blockNumber *big.Int) {
	stateObject := self.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.UpdateSmiloPay(blockNumber)
	}
}
