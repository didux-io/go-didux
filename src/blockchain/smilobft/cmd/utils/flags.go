...
import (
	...
	"go-didux/src/blockchain/smilobft/graphql"
	...
	"go-didux/src/blockchain/smilobft/consensus/clique"
...
var (
	...
	SportRequestTimeoutFlag = cli.Uint64Flag{
		...
		Value: eth.DefaultConfig.Sport.RequestTimeout,
	}
	SportBlockPeriodFlag = cli.Uint64Flag{
		Name:  "smilobft.blockperiod",
	...
	SmiloCodeAnalysisPathFlag = cli.StringFlag{
		Name:  "smilocodeanalysispath",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		...
		if ctx.GlobalBool(SportFlag.Name) {
			return filepath.Join(path, "sport")
		}
		return path
	}
...
func setMiner(ctx *cli.Context, cfg *miner.Config) {
	...
	log.Info("********* MINER ****** ", "ExtraData", cfg.ExtraData)
}
...
func setCodeQuality(ctx *cli.Context, cfg *eth.Config) {
	...
	if ctx.GlobalIsSet(SmiloCodeAnalysisPathFlag.Name) {
		cfg.SmiloCodeAnalysisPath = ctx.GlobalString(SmiloCodeAnalysisPathFlag.Name)
	}
}
