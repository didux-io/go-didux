...
func getIsSmilo(file io.Reader) (isSmilo, isGas, isGasRefunded bool) {
	targetStruct := new(struct {
		Config *struct {
			...
			IsGasRefunded *bool `json:"isGasRefunded"`
		} `json:"config"`
	})

	if err := json.NewDecoder(file).Decode(targetStruct); err != nil {
		log.Error("Could not decode file with targetStruct, ", "file", file, "targetStruct", targetStruct)
		utils.Fatalf("getIsSmilo, invalid genesis file: %v", err)
	}

	isSmilo = targetStruct.Config.IsSmilo == nil || *targetStruct.Config.IsSmilo

	isGas = targetStruct.Config.IsGas == nil || *targetStruct.Config.IsGas

	isGasRefunded = targetStruct.Config.IsGasRefunded == nil || *targetStruct.Config.IsGasRefunded

	return isSmilo, isGas, isGasRefunded
}
