...
type Wallet struct {
	...
	deriveChain     smilobft.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain smilobft.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
