...
func (b *LesApiBackend) StateAndHeaderByNumber(ctx context.Context, number rpc.BlockNumber) (vm.SmiloAPIState, *types.Header, error) {
	header, err := b.HeaderByNumber(ctx, number)
	if err != nil {
		return nil, nil, err
	}
...
func (b *LesApiBackend) GetEVM(ctx context.Context, msg core.Message, apiState vm.SmiloAPIState, header *types.Header, vmCfg vm.Config) (*vm.EVM, func() error, error) {
	statedb := apiState.(*state.StateDB)
	statedb.SetBalance(msg.From(), math.MaxBig256, header.Number)
...
func (b *LesApiBackend) GetSolcPath() string {
	solcpath := b.eth.config.SolcPath
	return solcpath
}

func (b *LesApiBackend) GetSmiloCodeAnalysisPath() string {
